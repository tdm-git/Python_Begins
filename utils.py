...
def currency_rates(*args):
    ...
    str_data = list_val[0].split('Date="')[1][:10]
    server_date = datetime.date(int(str_data[6:]), int(str_data[3:5]), int(str_data[:2]))
    dict_val = {}
    for i in list_val[1:]:
...
if __name__ == '__main__':  # обязательный блок для внешнего модуля
